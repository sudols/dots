...
# Keep running even if audio is unavailable at startup; we emit fallback JSON instead of exiting.
set -uo pipefail
...
# Main logic: always emit JSON, retry if sink missing or pactl dies
while true; do
  ...
  # If we exit the inner loop (e.g., due to missing sink or pactl stopping), retry.
  sleep 1
done
